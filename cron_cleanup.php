...
try {
    ...
    // Security fix: don't echo the raw DB exception message in the HTTP
    // response — this endpoint is gated by CRON_SECRET, but that's a
    // string in a .env file, not a guarantee. Full detail still goes to
    // cron_log.txt above for whoever is actually debugging this.
    echo "Cleanup failed. See cron_log.txt for details.\n";
    exit(1);
}
